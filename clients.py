...
OWM_API_TOKEN = os.getenv('OWM_TOKEN')
...
async def get_weather(city):
    ...
    """
    url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={OWM_API_TOKEN}&units=metric"
...
async def get_food_info(product_name):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            ...
            print(f"Ошибка при получении информации о продукте: {response.status}")
            return {
                'info': "Продукт не найден, будет использовано среднее значение 50 ккал на 100 г",
                'calories': 50
            }
